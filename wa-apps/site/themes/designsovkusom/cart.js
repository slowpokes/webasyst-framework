...
*/
    ...
    $(".cart input.qty").change(function () {
        ...
        if (that.val() > 0) {
            ...
            if (that.val()) {
                $.post('save/', {html: 0, id: tr.data('id'), quantity: that.val()}, function (response) {
    ...
    $(".cart .services input:checkbox").change(function () {
        ...
        if ($(this).is(':checked')) {
           ...
           var data = {html: 0, parent_id: parent_id, service_id: $(this).val()};
           ...
           $.post('delete/', {html: 0, id: div.data('id')}, function (response) {
               div.data('id', null);
               tr.find('.item-total').html(response.data.item_total);
               updateCart(response.data);
           }, "json");
    ...
    $(".cart .services select").change(function () {
        var tr = $(this).closest('tr');
        $.post('save/', {html: 0, id: $(this).closest('div').data('id'), 'service_variant_id': $(this).val()}, function (response) {
            tr.find('.item-total').html(response.data.item_total);
            updateCart(response.data);
        }, "json");
    });
