...
class bannersImage
{
    ...
    public function __construct($id){
        ...
        $this->model = new bannersImageModel();
        $this->getInfo();
    }
    ...
    public function scale(){
        $config = wa('banners')->getConfig();
        $banner = new bannersBanner($this->banner_id);
    ...
    public static function create($banner_id, waRequestFile $file){
        ...
        $image = new bannersImage($image_id);
        return $image;
    }

    public static function getPath($image){
        $folder = wa()->getDataPath(bannersBanner::getFolder($image['banner_id']), true, 'banners');
    ...
    public static function getThumbsUrl($image, $sizes){
        $folder = wa()->getDataUrl(bannersBanner::getFolder($image['banner_id']), true, 'banners', true);
        return $folder."{$sizes['width']}x{$sizes['height']}/{$image['id']}.{$image['ext']}";
    }
    ...
    public static function addClicks(&$images){
        ...
        $model = new bannersClickModel();
        $sql = 'SELECT image_id, count(id) as c FROM `banners_click` where image_id IN ('.implode(',', $ids).') group by image_id';
